...
function uncache(module = '.') {
   ...
   const alpha = alphaConnect({
      ...
      browser: ['WhatsApp PrivateSC', 'safari', '1.0.0'],
      auth: state
   })
   ...
   alpha.ev.on('messages.upsert', async chatUpdate => {
      ...
      try {
         ...
         for (let mek of chatUpdate.messages) {
            ...
            const isCreator = ["6281327893987@s.whatsapp.net", "6281327893987@s.whatsapp.net", ...global.owner].map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
